<table class="table--projects">
	@if (isset($seriesHead) && $seriesHead === true)
	<thead>
		<tr>
	...
	@endif
 	<tbody>
@if (isset($notYet) && $notYet === true)
		<tr>
			<td data-label="Title" class="td--project_title{{ $seriesArray['title'] == '' && $seriesArray['subtitle'] == '' ? ' td--empty' : '' }}">{!! $seriesArray['title'] != '' ? '<h1><cite>'.$seriesArray['title'].'</cite></h1>' : '' !!}{!! isset($seriesArray['subtitle']) ? '<h2 class="h2--subtitle">('.$seriesArray['subtitle'].')</h2>' : '' !!}</td>
...
@else
		<tr>
			...
			<td data-label="Title" class="td--project_title{{ $seriesArray['title'] == '' && $seriesArray['subtitle'] == '' ? ' td--empty' : '' }}"><a href="/o__o/{{ $seriesArray['slug'] }}/">{!! $seriesArray['title'] != '' ? '<h1><cite>'.$seriesArray['title'].'</cite></h1>' : '' !!}{!! isset($seriesArray['subtitle']) ? '<h2 class="h2--subtitle">('.$seriesArray['subtitle'].')</h2>' : '' !!}</a></td>
		@if (isset ($seriesArray['pages']))
			<td data-label="Subpages" class="td--series_subpages">
				<ul>
			@foreach ($seriesArray['pages'] as $page)
