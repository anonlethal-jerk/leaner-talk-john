...
hogs.forEach(function(hog) {
	...
	var t_speed = Math.round(Math.random()*15 + 10);
	var t_shadow_speed = Math.round(Math.random()*6 + 1);
	var move_hog = setInterval(function(){
		...
		var t_width = Math.random()*2;
		var t_height = Math.random()*2;
		hog.style.padding = t_height+"rem "+t_width+"rem";
	...
	var move_shadow = setInterval(function(){
		...
		// t_green = t_red;
		// t_blue = t_red;
		var t_offset_x = Math.round(Math.random()*200 - 100);
		var t_offset_y = Math.round(Math.random()*200 - 100);
