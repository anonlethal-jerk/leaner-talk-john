...
$series_info = [
	...
	'description' => 'People have submitted '.$submissions->count().' of their own creations. Send me an email with yours!',
	'notes'       => '',
];
@endphp
...
<!-- get rid of spacing
			...
			--><figure>
					<a href="{{ $boxbot->slug }}">
					<img class="{{ $toggleOther }} lazyload" src="/x__x/loading-99.gif" data-src="/o__o/boxbots/submissions/x__x/{{ $toggleOther }}-small/{{ $boxbot->imgfile }}.{{ $toggleOther == 'color' ? 'jpg' : 'png' }}" width="{{ $boxbot->percenthalfwidthpx }}" height="{{ $boxbot->percenthalfheightpx }}" alt="{{ $boxbot->title }} BoxBot" title="{{ $boxbot->title }}, {{ $boxbot->nice_date }}, {{ $boxbot->height }} × {{ $boxbot->width }} mm">
					<img class="{{ $toggleCurrent }} lazyload" src="/x__x/loading-99.gif" data-src="/o__o/boxbots/submissions/x__x/{{ $toggleCurrent }}-small/{{ $boxbot->imgfile }}.{{ $toggleCurrent == 'color' ? 'jpg' : 'png' }}" width="{{ $boxbot->percenthalfwidthpx }}" height="{{ $boxbot->percenthalfheightpx }}" alt="{{ $boxbot->title }} BoxBot" title="{{ $boxbot->title }}, {{ $boxbot->nice_date }}, {{ $boxbot->height }} × {{ $boxbot->width }} mm">
				</a>
				<div class="sort--hide">
